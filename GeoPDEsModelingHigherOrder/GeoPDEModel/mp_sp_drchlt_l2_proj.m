...
if length(refs)>1
    error('This function only works properly with ONE boundary specified.')
else
    ...
    for iref = refs
        for bnd_side = 1:boundaries(iref).nsides
            ...
            if (size (msh_bnd.geo_map, 1) == 2)
                ...
                hval = reshape (h (x, y, iref, bnd_side, boundaries(iref).nsides), sp_bnd.ncomp, msh_bnd.nqn, msh_bnd.nel);
            ...
            elseif (size (msh_bnd.geo_map, 1) == 3)
                ...
                hval = reshape (h (x, y, z, iref, bnd_side, boundaries(iref).nsides), sp_bnd.ncomp, msh_bnd.nqn, msh_bnd.nel);
            end
