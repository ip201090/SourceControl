...
function [ ] = createRabiTypeSternGerlachFEMMOptimized(type, turns, current, meshSize, folder, fileName, lengthFromOuterLimit)
% Add femm path to MATLAB
addpath C:\femm42\mfiles\

load nurbsCurves.mat;
poleResolution=50;
...
%% Draw geometry for weak type
% Add the nodes for convex tip
knotSpacing=linspace(0,0.2219,poleResolution);
pointsConvexNrb=nrbeval(convexnrb, knotSpacing);

for i=1:poleResolution
    mi_addnode(pointsConvexNrb(1,i), pointsConvexNrb(2,i));
end
...
% Add segments to connect nodes
for i=1:poleResolution-1
    mi_addsegment(pointsConvexNrb(1,i), pointsConvexNrb(2,i), pointsConvexNrb(1,i+1), pointsConvexNrb(2,i+1));
end

mi_addsegment(point4(1), point4(2), point5(1), point5(2));
...
% Add the nodes for concave tip
pointsConcaveNrb=nrbeval(concavenrb, knotSpacing);
for i=1:poleResolution
    mi_addnode(pointsConcaveNrb(1,i), pointsConcaveNrb(2,i));
end
...
% Add segments to connect nodes
for i=1:poleResolution-1
    mi_addsegment(pointsConcaveNrb(1,i), pointsConcaveNrb(2,i), pointsConcaveNrb(1,i+1), pointsConcaveNrb(2,i+1));
end
