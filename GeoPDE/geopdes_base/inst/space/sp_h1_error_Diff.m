...
function [errh1] = sp_h1_error_Diff (sp, msh, u, sp2, u2, geo2)
  ...
  for iel = 1:msh.nel_dir(1)
    ...
    grad_valu2 = reshape (sp_eval (u2, sp2, geo2, {qnu_tmp(:,iel) reshape(msh.qn{2},1,[])}, 'gradient'), sp.ncomp, ndim, msh_col.nqn, msh_col.nel);
...
%     grad_valex  = reshape (feval (graduex, x{:}), sp.ncomp, ndim, msh_col.nqn, msh_col.nel);
    ...
    errh1s = errh1s + sum (sum (reshape (sum (sum ...
       ((grad_valu - grad_valu2).^2, 1), 2), [msh_col.nqn, msh_col.nel]) .* w));
