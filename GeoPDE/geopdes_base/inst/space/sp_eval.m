...
function [eu, F] = sp_eval (u, space, geometry, npts, varargin)
  ...
  if (ndim == 2)
    msh = msh_2d ({brk{1}, brk{2}}, pts, [], geometry, 'boundary', false, 'der2', true);
  elseif (ndim == 3)
  ...
  switch (lower (option))
    ...
    case {'hessian'}
      [euGrad, euHess, F] = sp_eval_hessianSpecial_msh (u, sp, msh);
      F  = reshape (F, [ndim, npts]);
      euHess = squeeze (reshape (euHess, [sp.ncomp, ndim, ndim, npts]));
      euGrad = squeeze (reshape (euGrad, [sp.ncomp, ndim, npts]));
      eu=struct('euHess', euHess, 'euGrad', euGrad);
